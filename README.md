...
A single-backend, single-UI platform that shows two sides of the same MongoDB Atlas digital twin: **field-service diagnostics** and **aftermarket sales intelligence**. Both use cases share one database, one MCP server, one seed command, and one React application.

| Use Case | Audience | Tab | What it answers |
|----------|----------|-----|-----------------|
| **Virtual Engineer** | Field service engineers | VE section | *"What's wrong with this unit?"* |
| **Aftermarket Sales Intelligence** | Sales representatives | AMS section | *"What can I sell to this customer, and why now?"* |
| **Platform** | Leadership / architects | Platform tab | *"How does the shared architecture scale to new agents?"* |
...
```
Browser — PULSE UI (:5174)
  ├── Virtual Engineer section   SSE /api/chat
  ├── AMS section                SSE /api/sales/chat
  └── Platform tab               (static — architecture story)
          │
          ▼
  Express Server :3100
...
The PULSE UI proxies `/api` → `:3100`. The original `virtual_engineer` repo frontend also connects with zero config change (same port, same endpoint, same tools).

---

## MCP Tool Suite (36 total)

### Shared Foundation Tools (6)
Used by both the VE and Sales agents — demonstrates how new agents reuse existing tooling:

| Category | Tools |
|----------|-------|
| Asset & site | `getChillerById`, `getChillerConfiguration`, `getSiteContext` |
| Telemetry | `getTelemetry`, `getCurrentDeviceState` |
| Service history | `getServiceHistory` |

### Virtual Engineer — diagnostic tools (14)

| Category | Tools |
|----------|-------|
| Alarms | `getActiveAlarms`, `getAlarmHistory`, `getAlarmDetails` |
| Fault history | `getPartsHistory`, `getFaultEvents` |
...
| Feature | Used by |
|---------|---------|
| Standard Find / CRUD, `$lookup` joins | Both agents |
| Aggregation Pipeline (`$group`, `$match`, `$project`, `$unwind`, `$sort`) | Both agents |
...
|----------|------|---------|-------------|
| **Efficiency Drift** | CH-ATL-001 | kW/ton +9% over 180 days → refrigerant recharge + coil clean | `$setWindowFields` rolling avg |
...
Generates 1,710 telemetry docs per run (180-day daily + 7-day hourly per unit), seeds all 12 collections including the 5 AMS-only collections, and creates all indexes.
...
> Without search indexes, knowledge/catalog tools return `degraded: true` — all deterministic tools continue to work.
...
Starts backend on `:3100` and the PULSE UI on `:5174` concurrently. Both agents and all three tabs (VE, AMS, Platform) are available at **http://localhost:5174**.
...
| `backend/src/lib/searchIndexes.js` | Centralized Atlas Search index definitions |
...
The original `virtual_engineer` repo frontend proxies `/api` → `localhost:3100`. Since PULSE also runs on `:3100` with all 20 original VE tools and `/api/chat` preserved exactly:
