...
export function QueryInspector({ insight, title, result }: Props) {
  ...
  return (
    <div>
      ...
      {insight.rerank && (
        <div className="rank-fusion-legs" style={{ marginBottom: "0.5rem" }}>
          <PatternBadge pattern="voyage_rerank" />
          <span className="rank-fusion-leg" style={{ background: "#e8faf0", color: "#00684a" }}>
            {insight.rerank.model} · {insight.rerank.numDocsToRerank} docs · {insight.rerank.path}
          </span>
        </div>
      )}
