...
app.get("/health", async (_req, res) => {
  try {
    ...
    res.json({
      status: "ok",
      service: "pulse-equipment-platform",
      tools: MCP_TOOL_COUNT,
      chat: "/api/chat",
      sales_chat: "/api/sales/chat",
    });
  } catch (err) {
...
app.use("/api/sales", salesChatRouter);
...
const httpServer = app.listen(env.mcpPort, env.mcpHost, async () => {
  try {
    ...
    console.log(`  Sales API:    POST http://localhost:${env.mcpPort}/api/sales/chat`);
