...
import { buildRerankQuery } from "./rerank.js";
...
function hybridExcerpt({ rankFusion, rerank }) {
  if (!rerank) return excerpt(rankFusion);
  return excerpt([
    rankFusion,
    {
      $rerank: {
        model: rerank.model,
        query: { text: rerank.queryText },
        path: rerank.path,
        numDocsToRerank: rerank.numDocsToRerank,
      },
    },
  ]);
}

export function insightForKnowledgeSearch(query, type, filters, scoreDetails = null, rerank = null) {
  const rankFusion = {
    $rankFusion: {
      input: {
        pipelines: {
  ...
  return {
    ...
    query_excerpt: hybridExcerpt({
      rankFusion,
      rerank: rerank
        ? { ...rerank, queryText: buildRerankQuery(query, filters || {}) }
        : null,
    }),
    rank_fusion_legs: toRankFusionLegs(scoreDetails),
    ...(rerank ? { rerank: { model: rerank.model, path: rerank.path, numDocsToRerank: rerank.numDocsToRerank } } : {}),
  };
}

export function insightForCaseSearch(query, filters, scoreDetails = null, rerank = null) {
  const rankFusion = {
    $rankFusion: {
      input: {
        pipelines: {
