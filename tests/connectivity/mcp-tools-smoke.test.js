...
describe("MCP connectivity", () => {
  ...
  it("lists tools including getChillerById", async () => {
    ...
    assert.ok(!names.includes("rerankKnowledgeResults"));
    assert.ok(!names.includes("rerankSimilarCases"));
  });
  ...
  it("searchManuals uses $rankFusion then Voyage $rerank (requires knowledge indexes Active; $rerank needs MongoDB 8.3+)", async () => {
    const sessionId = await createMcpSession();
    ...
    if (parsed.query_insight.rerank) {
      assert.ok(parsed.query_insight.query_excerpt.includes("$rerank"));
      assert.ok(parsed.query_insight.rerank.model);
      assert.equal(parsed.query_insight.rerank.path, "rerank_text");
    }
    if (parsed.data?.results?.[0]?.rerank_score != null) {
      assert.equal(typeof parsed.data.results[0].rerank_score, "number");
    }
  });

  it("searchCaseNotes uses $rankFusion then Voyage $rerank (requires service_tickets indexes Active; $rerank needs MongoDB 8.3+)", async () => {
    const sessionId = await createMcpSession();
